...
const Dashboard = () => {
  ...
  const recentBookings = [
    { id: "B001", service: "Housemaid", client: "Sarah Ahmed", status: "Active", time: "2 hours ago" },
    { id: "B002", service: "Driver", client: "Mohammed Ali", status: "Pending", time: "4 hours ago" },
    { id: "B003", service: "Chef", client: "Fatima Hassan", status: "Completed", time: "6 hours ago" },
    { id: "B004", service: "Housemaid", client: "Ahmed Omar", status: "Active", time: "8 hours ago" }
  ];
      ...
      <div className="bg-gradient-card min-h-full">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8 animate-fade-in">
            <h1 className="text-3xl font-bold text-gray-900">{t('welcomeToDashboard')}</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {stats.map((stat, index) => (
              <Card key={stat.title} className="hover:shadow-card transition-all duration-300 animate-scale-in" style={{ animationDelay: `${index * 100}ms` }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card className="lg:col-span-2 animate-slide-in-right">
              <CardHeader>
                ...
                <CardDescription>Current staff distribution across services</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  {services.map((service, index) => (
                    <div key={service.name} className="flex items-center p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer" style={{ animationDelay: `${index * 150}ms` }}>
                      <div className={`p-2 rounded-lg ${service.color} mr-3`}>
                        <service.icon className="w-5 h-5 text-white" />
                      </div>
                      <div>
                        <p className="font-semibold text-gray-900">{service.count}</p>
                ...
                <div className="space-y-4">
                  {recentBookings.map((booking) => (
                    <div key={booking.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                      ...
                      <Badge 
                        variant={booking.status === "Active" ? "default" : booking.status === "Pending" ? "secondary" : "outline"}
                        className="text-xs"
                      >
                        {booking.status}
                      </Badge>
          ...
          {/* Quick Actions */}
          <Card className="mt-8 animate-fade-in" style={{ animationDelay: "500ms" }}>
            <CardHeader>
              <CardTitle>Quick Actions</CardTitle>
              <CardDescription>Manage your SAAT CRM efficiently</CardDescription>
            </CardHeader>
            <CardContent>
